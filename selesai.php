...
if ($simpan) {
    ...
    $tersimpan = mysqli_query($koneksi, "SELECT * FROM nilai where jml_benar<>0 and id_ujian='$idu' and id_mapel='$idm' and id_siswa='$ids'");

    if (mysqli_num_rows($tersimpan) > 0) {
        mysqli_query($koneksi, "DELETE FROM jawaban where jawaban.id_ujian='$idu' and jawaban.id_mapel='$idm' and jawaban.id_siswa='$ids' ");
    }
